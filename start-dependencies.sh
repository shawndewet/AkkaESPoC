...
if docker run -e "ACCEPT_EULA=Y" -e "SA_PASSWORD=yourStrong(!)Password" -p 1533:1433 --name "AkkaESPoC-sql" -d "${imageName}:${version}" ; then
	...
	if ../../../infrastructure/build.all.sh $1 ; then
		...
		return -1
		docker run -e "ACCEPT_EULA=Y" -e "SA_PASSWORD=yourStrong(!)Password" -p 1533:1433 --name "AkkaESPoC-sql" -d "${imageName}:${version}"
	fi
fi